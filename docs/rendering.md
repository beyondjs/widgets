...
- destroy removes its GlobalCSS change callback and releases the GlobalCSS subscription to the runtime's stylesheet registry. It does not remove links, clear own consumers or reject a pending ready promise; the client controller's `dispose` releases the runtime's DependenciesStyles.

[GlobalCSS](../src/modules/render/web/widget/styles/global.ts) addresses the shared stylesheet as `<host>/styles/global<query>` of the package of the widget and adopts it inside the root of every widget of the package, before the widget's own sheets. The compiler declares in the registration of a widget whether its package publishes a `./global` module, so a package without one links nothing and requests nothing; the preparation of a published package reaches that sheet from every widget of the package, so a widget delivered alone has it. The address is registered once in the runtime's stylesheet registry (`styles.link`, of `@beyond-js/kernel/styles` mapped to the development runtime), under the module it belongs to (`<package>@<version>/global`): every widget of the package shares that entry, and a development update of the shared sheet replaces it there, so every root that adopted it links the new version while a failed one keeps the last loaded version. A runtime without `styles.link` (the legacy Kernel) keeps a version of its own, which `update()` increments.

The stylesheets a module selects by specifier (`import '@scope/ui/theme.css'`) are adopted the same way: the runtime registers them with the module and `DependenciesStyles` collects them before the module's own sheet, so a widget adopts in its root what it and its non-widget dependencies select. Nothing is adopted because of the `.css` extension alone: the relation is what the compiler declares.
